...
4. Clean up entities with the integration test prefix

Usage:
    python scripts/cleanup_integration_resources.py [--dry-run] [--entity-prefix PREFIX]
...
async def cleanup_entities_with_prefix(
    ...
    """Clean up entities matching a prefix across all stores."""
    results: dict[str, list[str]] = {"dropped": [], "failed": [], "found": []}

    logger.info("=" * 60)
    logger.info("Cleaning entities with prefix: %s", prefix)
    logger.info("=" * 60)

    conn = _get_deltastream_connection()
    stores = await _list_stores(conn)
...
def cleanup_all(entity_prefix: str, dry_run: bool = False) -> None:
    ...
    if not databases:
        ...
        if not dry_run:
            for db_name in databases:
                ...
                # Reuse clean_database_with_children from test_helpers.py
    ...
    # Step 2: Clean up entities with IT prefix
    logger.info("Step 2: Cleaning entities with prefix: %s", entity_prefix)
    entity_results = run_async_safely(
        cleanup_entities_with_prefix(entity_prefix, dry_run)
    )
