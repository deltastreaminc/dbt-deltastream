...
class DeltastreamConnectionManager(BaseConnectionManager):
    ...
    def _extract_pending_files(self, sql: str, pending_files: Dict[str, str]) -> List[str]:
        """
        ...
        for sql_pattern, key_prefix in file_mappings:
            if sql_pattern in sql:
                ...
                matching_keys = [key for key in pending_files.keys() if key.startswith(key_prefix)]
    ...
    def _query_with_function_retry(self, sql: str, max_wait_seconds: int = 30) -> Tuple[AdapterResponse, "agate.Table"]:
        ...
        while True:
            try:
                ...
                if self._is_function_source_not_ready_error(e) and elapsed_time < max_wait_seconds:
                    logger.info(f"Function source not ready (SQLState: {e.code}), waiting {retry_interval}s before retry... (elapsed: {elapsed_time:.1f}s)")
                    ...
                    if elapsed_time >= max_wait_seconds and self._is_function_source_not_ready_error(e):
                        logger.error(f"Function source still not ready after {max_wait_seconds}s, giving up (SQLState: {e.code})")
                    raise
            except Exception as e:
                ...
                if hasattr(e, '__cause__') and isinstance(e.__cause__, SQLError):
                    ...
                    if self._is_function_source_not_ready_error(sql_error) and elapsed_time < max_wait_seconds:
                        logger.info(f"Function source not ready (SQLState: {sql_error.code}), waiting {retry_interval}s before retry... (elapsed: {elapsed_time:.1f}s)")
                        time.sleep(retry_interval)
                        retry_interval = min(retry_interval * 1.5, max_retry_interval)
                        continue
                    elif elapsed_time >= max_wait_seconds and self._is_function_source_not_ready_error(sql_error):
                        logger.error(f"Function source still not ready after {max_wait_seconds}s, giving up (SQLState: {sql_error.code})")
