"""
Test entity materialization functionality for deltastream adapter.

This test verifies that the adapter can create entities for Kafka topics.
Entities are schema definitions in DeltaStream.

Resources are automatically cleaned up by the session-level fixture in conftest.py.
"""

import pytest
from datetime import datetime
from dbt.tests.util import write_file
from tests.functional.adapter.test_helpers import run_dbt_with_retry


class TestCreateEntityDeltastream:
    """Test entity materialization functionality specific to DeltaStream."""

    @pytest.mark.integration
    def test_create_entity_from_shipments_topic(
        self,
        project,
        integration_database,
        integration_schema,
        integration_entity_names,
    ):
        """Test creating an entity for shipments topic from trial_store."""
        # Generate timestamp suffix for unique entity name
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S_%f")[:-3]
        # Get prefix from integration_entity_names
        prefix = integration_entity_names["pageviews"].replace("pageviews", "")
        entity_name = f"{prefix}it_shipments_entity_{timestamp}"

        sources_yml = f"""
version: 2

sources:
  - name: integration_tests
    database: {integration_database}
    schema: {integration_schema}
    tables:
      - name: {entity_name}
        description: "Shipments entity for integration test"
        config:
          materialized: entity
        columns:
          - name: shipment_id
            type: VARCHAR
          - name: order_id
            type: VARCHAR
          - name: warehouse_id
            type: VARCHAR
          - name: shipment_time
            type: BIGINT
          - name: carrier
            type: VARCHAR
          - name: delivery_status
            type: VARCHAR
""".lstrip()

        write_file(sources_yml, project.project_root, "models", "sources.yml")

        # Run the operation to create sources
        run_dbt_with_retry(["run-operation", "create_sources"], expect_pass=True)

        # Session-level cleanup will handle resource deletion
